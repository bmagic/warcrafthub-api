    ...
    request(url, function (error, response, body) {
        if (!error && response.statusCode === 200) {
            try {
                ...
                return callback(new Error("Error when parsing warcraftlogs response."));
            }
            callback(error, warcraftlogs);
        }
        else {
            if (error) {
                callback(new Error(error.message + " on fetching warcraftlogs api " + url));
            } else {
                callback(new Error("Error HTTP " + response.statusCode + " on fetching warcraftlogs api " + url));
            }
        }
    });
};
