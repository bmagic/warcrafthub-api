...
module.exports.getGuild = function (region, realm, name, params, callback) {
    ...
    var endUrl = "wow/guild/" + realm + "/" + name + "?fields=" + params.join(',') + "&locale=en_GB&apikey=" + config.bnet.clientID;
...
module.exports.getCharacter = function (region, realm, name, params, callback) {
    var config = applicationStorage.config;
    var endUrl = "wow/character/" + realm + "/" + name + "?fields=" + params.join(',') + "&locale=en_GB&apikey=" + config.bnet.clientID;
    this.requestBnetApi(region, endUrl, function (error, statusCode) {
        callback(error, statusCode);
    });
};
...
module.exports.getRealms = function (region, callback) {
    var config = applicationStorage.config;
    var endUrl = encodeURI("wow/realm/status?locale=en_GB&apikey=" + config.bnet.clientID);
    this.requestBnetApi(region, endUrl, function (error, result) {
...
module.exports.getAuctions = function (region, realm, callback) {
    ...
    async.waterfall([
        function (callback) {
            var endUrl = encodeURI("wow/auction/data/" + realm + "?locale=en_GB&apikey=" + config.bnet.clientID);
...
module.exports.request = function (url, callback) {
    ...
    logger.silly('GET BNET API : %s', url);
